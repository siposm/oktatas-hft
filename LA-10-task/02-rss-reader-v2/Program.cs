    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < tasks.Length; i++)
            {
                int _i = i; // OVT!!!
                tasks[i] = Task
                    .Run(() => DataProcessor.Download(_i, CTS.Token), CTS.Token)
                    .ContinueWith(x =>
                        {
                            Console.WriteLine("Task has finished successfully. Task status: " + x.Status);

                        }, TaskContinuationOptions.OnlyOnRanToCompletion)
                    .ContinueWith(x =>
                        {
                            Console.WriteLine("Task was canceled. Task status: " + x.Status);
